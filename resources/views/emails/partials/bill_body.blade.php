            ...
            <p class="left" style="line-height: 22px; margin: 3px 0 0; padding: 0;">
                @if ($bill->due_date)
                    <span style="font-size: 11px; color: #8f8d8e;">
                                    @if ($bill->isQuote())
                            {{ strtoupper(trans('texts.valid_until')) }} {{ $account->formatDate($bill->due_date) }}
                        @else
                            @if ($account->hasCustomLabel('due_date'))
                                {{ $account->getLabel('due_date') }} {{ $account->formatDate($bill->partial_due_date ?: $bill->due_date) }}
                            @else
                                {{ utrans('texts.due_by', ['date' => $account->formatDate($bill->partial_due_date ?: $bill->due_date)]) }}
                            @endif
                        @endif
                                </span><br/>
                @endif
            ...
            <p style="margin: 0; padding: 0;">
                @if (! isset($isRefund) || ! $isRefund)
                    <span style="font-size: 12px; color: #8f8d8e;">
                                    {{ strtoupper(trans('texts.' . $bill->present()->balanceDueLabel)) }}:
                                </span>
                    <br/>
                    <span class="total" style="font-size: 27px; color: #FFFFFF; margin-top: 5px;display: block;">
                                    {{ $account->formatMoney($bill->getRequestedAmount(), $vendor) }}
                                </span>
                @endif
            </p>
        </td>
