<table class="table invoice-table {{ $isTasks ? 'task-table' : 'product-table' }}">
    ...
    <tr>
        ...
        <th style="min-width:{{ $account->enable_second_tax_rate ? 180 : 120 }}px;display:none;"
    ...
    <tr data-bind="event: { mouseover: showActions, mouseout: hideActions }" class="sortable-row">
        ...
        <td>
            ...
            <input type="text"
                   ...
                   style="display: none"/>
            <input type="text"
                   data-bind="value: expense_public_id, attr: {name: 'invoice_items[{{ $isTasks ? 'T' : '' }}' + $index() + '][expense_public_id]'}"
                   style="display: none"/>
            <input type="text"
                   data-bind="value: invoice_item_type_id, attr: {name: 'invoice_items[{{ $isTasks ? 'T' : '' }}' + $index() + '][invoice_item_type_id]'}"
                   style="display: none"/>
        </td>
        @if ($account->customLabel('product1'))
            <td>
        ...
        <td style="display:none;" data-bind="visible: $root.invoice_item_taxes.show">
            {!! Former::select('')
                    ...
                    ->raw() !!}
            <input type="text"
                   data-bind="value: tax_name1, attr: {name: 'invoice_items[{{ $isTasks ? 'T' : '' }}' + $index() + '][tax_name1]'}"
                   style="display:none">
            <input type="text"
                   data-bind="value: tax_rate1, attr: {name: 'invoice_items[{{ $isTasks ? 'T' : '' }}' + $index() + '][tax_rate1]'}"
                   style="display:none">
            ...
            <input type="text"
                   data-bind="value: tax_name2, attr: {name: 'invoice_items[{{ $isTasks ? 'T' : '' }}' + $index() + '][tax_name2]'}"
                   style="display:none">
            <input type="text"
                   data-bind="value: tax_rate2, attr: {name: 'invoice_items[{{ $isTasks ? 'T' : '' }}' + $index() + '][tax_rate2]'}"
                   style="display:none">
        </td>
