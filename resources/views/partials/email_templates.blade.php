<script type="text/javascript">
    ...
    function renderEmailTemplate(str, invoice, entityType) {
        ...
        // Add any available payment method links
                ...
                {!! "keys['" . \App\Libraries\Utils::toCamelCase(\App\Models\GatewayType::getAliasFromId($type)) . "Link'] = '" . auth()->user()->account->getBaseUrl() . "/...';" !!}
                {!! "keys['" . \App\Libraries\Utils::toCamelCase(\App\Models\GatewayType::getAliasFromId($type)) . "Button'] = '" . Form::flatButton('pay_now', '#36c157') . "';" !!}
                @endif
                @endforeach
                                ...
                                <ul>
                                    ...
                                    @foreach (\App\Models\Gateway::$gatewayTypes as $type)
                                        @if ($account->getGatewayByType($type))
                                            @if ($type != GATEWAY_TYPE_TOKEN)
                                                <li>
                                                    ${{ \App\Libraries\Utils::toCamelCase(\App\Models\GatewayType::getAliasFromId($type)) }}
                                                    Link
                                                </li>
                                                <li>
                                                    ${{ \App\Libraries\Utils::toCamelCase(\App\Models\GatewayType::getAliasFromId($type)) }}
