...
@section('head')
    ...
    <link href="{{ asset('css/jquery.timepicker.css') }}?no_cache={{ NINJA_VERSION }}" rel="stylesheet"
          type="text/css"/>
...
@section('head_css')
    ...
    <style type="text/css">
        ...
        .list-group-item-type1:before {
            background-color: #1c9f77;
        }

        .list-group-item-type2:before {
            background-color: #d95d02;
        }

        .list-group-item-type3:before {
            background-color: #716cb1;
        }

        .list-group-item-type4:before {
            background-color: #e62a8b;
        }

        .list-group-item-type5:before {
            background-color: #5fa213;
        }

        .list-group-item-type6:before {
            background-color: #e6aa04;
        }

        .list-group-item-type7:before {
            background-color: #a87821;
        }

        .list-group-item-type8:before {
            ...
            <div class="navbar-collapse" style="padding-top:12px; padding-bottom:12px;">
                ...
                <div class="input-group input-group-lg">
                    <span class="input-group-addon" style="width:1%;"
                          data-bind="click: onFilterClick, style: { 'background-color': (filterState() != 'all' || filterStatusId() != '') ? '#ffffaa'  : '' }"
                          title="{{ trans('texts.filter_sort') }}"><span
                                class="glyphicon glyphicon-filter"></span></span>
                    <input id="search" type="search" class="form-control search" autocomplete="off"
                           autofocus="autofocus"
                    ...
                    <span class="input-group-addon" style="width:1%;" data-bind="click: onClearClick, visible: filter()"
                          title="{{ trans('texts.clear') }}">
						<span class="glyphicon glyphicon-remove"></span>
					</span>
                    <span class="input-group-addon" style="width:1%;"
                          data-bind="click: onRefreshClick, visible: ! filter()" title="{{ trans('texts.refresh') }}">
    ...
    <div class="container" style="margin: 0 auto;width: 100%;">
        <div class="row no-gutter">
            ...
            <div id="taskForm" class="col-sm-8 col-sm-push-4 col-md-7 col-md-push-5">
                <div id="formDiv" class="panel panel-default x-affix" data-bind="visible: selectedTask"
                     style="margin:20px; display:none;">
                    <div class="panel-body">
                        <form id="taskForm">
							<span data-bind="event: { keypress: onFormKeyPress }">
								<div class="row">
									<div style="padding-bottom: 20px; padding-right:6px;"
                                         class="client-select col-md-6 no-padding-mobile">
									...
									<div style="padding-bottom: 20px; padding-left:6px;"
                                         class="project-select col-md-6 no-padding-mobile">
								...
								@if (Auth::user()->hasFeature(FEATURE_INVOICE_SETTINGS))
                                    <div class="row">
										@if ($customLabel = $account->customLabel('task1'))
                                            <div style="padding-bottom: 20px; padding-right:6px;"
                                                 class="col-md-6 no-padding-mobile">
								...
								<table class="table times-table" data-bind="event: { change: selectedTask().onChange }"
										...
										<tr data-bind="event: { mouseover: onMouseOver, mouseout: onMouseOut }">
											...
											<td style="width: 38px; padding-top: 0px; padding-right: 8px"
												...
												<i style="cursor:pointer;float:right"
                                                   data-bind="click: $root.selectedTask().removeTime, visible: actionButtonVisible"
                                                   class="fa fa-minus-circle redlink"
                                                   title="{{ trans('texts.remove') }}"/>
											</td>
            ...
            <!-- Task List -->
            <div id="taskList" class="list-group col-sm-4 col-sm-pull-8 col-md-5 col-md-pull-7"
                 style="display:none;overflow:auto;margin-bottom:0px;">
                <div id="filterPanel"
                     style="margin-bottom:0px;padding-top:20px;padding-bottom:0px;padding-left:10px;display:none;">
                ...
                <div data-bind="foreach: filteredTasks">
                    <a href="#"
                       data-bind="click: $parent.selectTask, event: { mouseover: onMouseOver, mouseout: onMouseOut }, css: listItemState"
                       class="list-group-item">
                        <div class="pull-right" style="text-align:right;">
                            <div data-bind="visible: actionButtonVisible()"
                                ...
                                &nbsp;&nbsp;
                                <button type="button"
                                        data-bind="css: startClass, click: onStartClick, clickBubble: false"
                                        class="btn btn-sm"
                                        style="padding-left:0px; padding-right: 12px; padding-bottom: 6px; margin-top:5px;">
                                    <span data-bind="css: startIcon"></span>
                                </button>
                            </div>
                        </div>
                        <div class="pull-right" style="text-align:right; padding-left: 16px;">
                            ...
                            <div data-bind="text: age, style: { fontWeight: isRunning() ? 'bold' : '' }"
                        ...
                        <div style="white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">
                            ...
                            <p class="list-group-item-text">
                                <span class="link"
                                      data-bind="text: clientName, click: $parent.viewClient, clickBubble: false, css: clientClass"></span>
                                <span data-bind="visible: clientName &amp;&amp; projectName"> | </span>
                                <span class="link"
                                      data-bind="text: projectName, click: $parent.viewProject, clickBubble: false, css: projectClass"></span>
                                &nbsp;
                            </p>
                        </div>
                    </a>
