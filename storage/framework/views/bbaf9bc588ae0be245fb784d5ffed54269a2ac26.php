...
<?php $__currentLoopData = $invoice->client->account->getFontFolders();
    ...
    <div class="container">
                ...
                <?php elseif ($invoice->client->account->isGatewayConfigured() && floatval($invoice->balance) && !$invoice->is_recurring): ?>
                    ...
                    <?php elseif (count($paymentTypes) == 1): ?>
                        <a href='<?php echo e($paymentURL); ?>'
                           class="btn btn-success btn-lg"><?php echo e(trans('texts.pay_now')); ?><?php echo $invoice->present()->gatewayFee($gatewayTypeId); ?></a>
                    <?php endif; ?>
				</span>
                <?php else: ?>
                    <?php echo Button::normal(trans('texts.download'))->withAttributes(['onclick' => 'onDownloadClick()'])->large(); ?>
        ...
        <?php if ($account->isPro() && $invoice->hasDocuments()): ?>
            <div class="invoice-documents">
                ...
                <ul>
                    <?php $__currentLoopData = $invoice->allDocuments();
                    $__env->addLoop($__currentLoopData);
                    foreach ($__currentLoopData as $document): $__env->incrementLoopIndices();
                        $loop = $__env->getLastLoop(); ?>
                        <li><a target="_blank"
                               href="<?php echo e($document->getClientUrl($invitation)); ?>"><?php echo e($document->name); ?>

                                (<?php echo e(Form::human_filesize($document->size)); ?>)</a></li>
                    <?php endforeach;
                    $__env->popLoop();
                    $loop = $__env->getLastLoop(); ?>
                </ul>
...
<?php if ($account->requiresAuthorization($invoice)): ?>
    <div class="modal fade" id="authorizationModal" tabindex="-1" role="dialog"
         aria-labelledby="authorizationModalLabel" aria-hidden="true">
