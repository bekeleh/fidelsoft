...
use App\Http\Requests\CreateInvoiceItemAPIRequest;
...
class InvoiceItemApiController extends BaseAPIController
{
     ...
     *      @SWG\Schema(type="array", @SWG\Items(ref="#/definitions/InvoiceItem"))
     *   ),
     *   @SWG\Response(
     *     response="default",
     ...
     *   summary="Retrieve an InvoiceItem",
     *   tags={"invoice"},
     *   @SWG\Parameter(
     *     in="path",
     *     name="invoice_id",
     ...
     *      @SWG\Schema(type="object", @SWG\Items(ref="#/definitions/InvoiceItem"))
     *   ),
     *   @SWG\Response(
     *     response="default",
     *     description="an ""unexpected"" error"
     ...
     * @return
     */
    public function show(InvoiceItemRequest $request)
    {
     ...
     *     in="body",
     *     name="invoice",
     *     @SWG\Schema(ref="#/definitions/InvoiceItem")
     *   ),
     *   @SWG\Response(
     *     response=200,
    ...
    public function emailInvoiceItem(InvoiceItemRequest $request)
    {
        $invoice = $request->entity();

        if ($invoice->is_recurring && $recurringInvoiceItem = $this->invoiceItemRepo->createRecurringInvoiceItem($invoice)) {
            $invoice = $recurringInvoiceItem;
        }
        ...
        if (config('queue.default') !== 'sync') {
            $this->dispatch(new SendInvoiceItemEmail($invoice, auth()->user()->id, $reminder, $template));
        } else {
            $result = app('App\Ninja\Mailers\ContactMailer')->sendInvoiceItem($invoice, $reminder, $template);
            if ($result !== true) {
     ...
     * @return
     */
    public function update(UpdateInvoiceItemRequest $request, $publicId)
    {
        if ($request->action == ACTION_CONVERT) {
            $quote = $request->entity();
            $invoice = $this->invoiceItemRepo->cloneInvoiceItem($quote, $quote->id);
        ...
        $invoice = InvoiceItem::scope($publicId)
            ->withTrashed()
     ...
     * @return
     */
    public function destroy(UpdateInvoiceItemRequest $request)
    {
        $invoice = $request->entity();
    ...
    public function download(InvoiceItemRequest $request)
    {
        $invoice = $request->entity();
