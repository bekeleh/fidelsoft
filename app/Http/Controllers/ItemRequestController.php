...
class ItemRequestController extends BaseController
{
    ...
    public function __construct(
        ItemRequestRepository $itemRequestRepo, ItemRequestService $itemRequestService)
    {
        // parent::__construct();
    ...
    {
        return $this->itemRequestService->getDatatableWarehouse($warehousePublicId);
    }
    ...
    private static function getViewModel($itemRequest = false)
    {
        return [
            ...
            'warehouses' => Warehouse::scope()->withActiveOrSelected($itemRequest
    ...
    public function approve(ItemRequest $request)
    {
        ...
        $requestedWarehouseId = Input::get('warehouse_id');
        ...
        if ($itemRequest) {
            ...
            if ($itemRequest->save()) {
                ItemRequest::quantityAdjustment($productId, $requestedWarehouseId, $itemRequest->qty, auth::user()->store->id, $deliveredQty);
            }
